...
class ListingControllerTest {
  ...
  private ListingResponse createTestResponse(
      Long id,
      String title,
      String brand,
      BigDecimal price,
    ...
    ListingResponse r =
        new ListingResponse(
            id, title, brand, price, condition, status, ownerId, LocalDateTime.now());
  ...
  void getListingById_ShouldReturn200() throws Exception {
    ListingResponse response =
        createTestResponse(
            1L, "iPhone 14", "Apple", new BigDecimal("999.99"), Condition.New, Status.Active, 1L);

    when(listingService.getListingById(1L)).thenReturn(response);

    mockMvc
        .perform(get("/api/listings/1"))
    ...
    mockMvc
        .perform(
            post("/api/listings/add")
                ...
                .content(
                    "{\"title\":\"MacBook Pro\",\"brand\":\"Apple\",\"price\":2499.99,\"condition\":\"New\"}"))
        .andExpect(status().isOk())
  ...
  void filterListings_ShouldReturnFilteredListings() throws Exception {
    ...
    when(listingService.filterListings(
            any(ListingFilterRequest.class), eq(0), eq(10), eq("createdAt"), eq("desc")))
        .thenReturn(page);
    ...
    mockMvc
        .perform(
            post("/api/listings/filter")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"brand\":\"Apple\",\"condition\":\"New\",\"minPrice\":100,\"maxPrice\":2000}")
                .param("page", "0")
                .param("size", "10"))
        .andExpect(status().isOk())
  ...
  void updateListing_ShouldReturnUpdatedListing() throws Exception {
    ListingResponse updated =
        createTestResponse(
            1L, "iPhone 15", "Apple", new BigDecimal("1199.99"), Condition.New, Status.Active, 1L);
    ...
    mockMvc
        .perform(
            put("/api/listings/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"title\":\"iPhone 15\",\"brand\":\"Apple\",\"price\":1199.99,\"condition\":\"New\"}"))
        .andExpect(status().isOk())
  ...
  void findByBrand_ShouldReturnListings() throws Exception {
    ...
    mockMvc
        .perform(get("/api/listings/brand/Apple"))
        .andExpect(status().isOk())
  ...
  void findByCondition_ShouldReturnListings() throws Exception {
    ...
    mockMvc
        .perform(get("/api/listings/condition/Used"))
        .andExpect(status().isOk())
  ...
  void findByPriceRange_ShouldReturnListings() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/listings/price-range").param("minPrice", "500").param("maxPrice", "1500"))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            post("/api/listings/add")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"title\":\"Test\",\"brand\":\"Test\",\"price\":100,\"condition\":\"New\"}"))
        .andExpect(status().isForbidden());
  }

  @Test
  void deleteListing_WithoutAuth_ShouldReturn403() throws Exception {
    mockMvc.perform(delete("/api/listings/1")).andExpect(status().isForbidden());
  }
  ...
  void getUserListings_WithoutAuth_ShouldReturn403() throws Exception {
    mockMvc.perform(get("/api/listings/user")).andExpect(status().isForbidden());
  }
}
