...
public class DealService {
  ...
  public DealResponse createPurchaseRequest(CreateDealRequest request, String buyerEmail) {
    ...
    if (listing.getOwner().getId().equals(buyer.getId())) {
      throw new RuntimeException("Нельзя откликнуться на своё же объявление");
    }

    List<DealStatus> activeStatuses = Arrays.asList(DealStatus.PENDING, DealStatus.APPROVED);

    if (dealRepository.existsActiveDealByListing(listing, activeStatuses)) {
      throw new RuntimeException("По этому объявлению уже есть активная сделка");
    }
  ...
  public DealResponse confirmDeal(Long dealId, String sellerEmail) {
    ...
    if (!deal.getSeller().getEmail().equals(sellerEmail)) {
      throw new RuntimeException("Только продавец может подтвердить сделку");
    }
    if (deal.getStatus() != DealStatus.PENDING) {
      throw new RuntimeException("Сделку можно подтвердить только в статусе PENDING");
    }
  ...
  public DealResponse completeDeal(Long dealId, String userEmail) {
    ...
    if (!deal.getSeller().getEmail().equals(userEmail)
        && !deal.getBuyer().getEmail().equals(userEmail)) {
      throw new RuntimeException("Только участники могут завершить сделку");
    }

    if (deal.getStatus() != DealStatus.APPROVED) {
      throw new RuntimeException("Сделку можно завершить только в статусе APPROVED");
    }
  ...
  public DealResponse cancelDeal(Long dealId, String userEmail) {
    ...
    if (!deal.getSeller().getEmail().equals(userEmail)
        && !deal.getBuyer().getEmail().equals(userEmail)) {
      throw new RuntimeException("Только участники могут отменить сделку");
    }

    if (deal.getStatus() == DealStatus.COMPLETED) {
      throw new RuntimeException("Нельзя отменить завершенную сделку");
    }
  ...
  public DealResponse getDealById(Long id, String userEmail) {
    Deal deal =
        dealRepository.findById(id).orElseThrow(() -> new RuntimeException("Сделка не найдена"));

    if (!deal.getSeller().getEmail().equals(userEmail)
        && !deal.getBuyer().getEmail().equals(userEmail)) {
      throw new RuntimeException("Вы не участник этой сделки");
    }
  ...
  // Все сделки юзера
  public Page<DealResponse> getUserDeals(
      String userEmail, int page, int size, String sortBy, String direction) {
    User user =
        userRepository
  ...
  // Конвертер с Deal в DealResponse. для безопасности. без этого пользователь получит то что не
  // должен видеть. чтото вроде этого
  // {
  //    "id": 1,
  //    "buyer": {
  //        "id": 2,
