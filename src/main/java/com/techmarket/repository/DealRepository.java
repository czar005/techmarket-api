...
public interface DealRepository extends JpaRepository<Deal, Long> {
  ...
  @Query(
      "SELECT d FROM Deal d WHERE d.listing = :listing AND d.status IN :statuses") // ищу активную
                                                                                   // сделку
  Optional<Deal> findActiveDealByListing(
      @Param("listing") Listing listing, @Param("statuses") List<DealStatus> statuses);

  @Query(
      "SELECT COUNT(d) > 0 FROM Deal d WHERE d.listing = :listing AND d.status IN :statuses") // проверка на наличие активной сделки
  boolean existsActiveDealByListing(
      @Param("listing") Listing listing, @Param("statuses") List<DealStatus> statuses);
